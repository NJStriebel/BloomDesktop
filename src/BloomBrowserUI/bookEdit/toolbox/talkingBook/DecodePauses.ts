...
//all functions in this file perform the same task of estimating splits
...
export function textLength(
    ...
    const totalTextLength: number = initialSegments.reduce(
        (lengthSoFar: number, thisSegment: TimedTextSegment) => {
            return lengthSoFar + thisSegment.text.length;
        },
        0
    );
    ...
    for (let i = 0; i < segments.length - 1; i++) {
        segments[i].end =
            (segments[i].text.length / totalTextLength) * duration;
...
export function quietestNearby(
    initialSegments: TimedTextSegment[],
    audioData: number[],
    duration: number
) {
    ...
    for (let i = 0; i < segments.length - 1; i++) {
        ...
        const slop =
            ((seg.end - seg.start) / duration) * slopPercent * audioData.length;
        ...
        const numberOfBreaks = Math.min(breakSpotCount, end - start); // paranoia, should always be breakSpotCount
